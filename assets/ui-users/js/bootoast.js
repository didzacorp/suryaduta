...
(function($, window, document, undefined) {
    ...
    function BootstrapNotify(options) {
        ...
        if (options !== undefined) {
            ...
            var title = '';
            if (this.settings.type == 'warning') {
                title ='Warning';
            }else if (this.settings.type == 'success') {
                 title ='Success';
            }else if (this.settings.type == 'danger') {
                 title ='Error';
            }else if (this.settings.type == 'info') {
                 title ='Info';
            }else{

            }
            // Adiciona o .alert ao .container conforme seu posicionamento.
            this.$el = $('<div class="alert alert-' + this.settings.type + ' ' + pluginName + '">'+
                        '<span style="font-weight: bold;">'+
                            '<i class="fa fa-' + this.settings.icon + '"></i> '+title+
                        '</span>'+
                            '<span class="bootoast-alert-container">'+
                                '<span class="bootoast-alert-content" style="padding-left: 6%;">'+
                                this.content +
                                '</span>'+
                            '</span>'+
                        '</div>')[this.putTo]('.' + containerClass + positionSelector);
